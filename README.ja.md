# MoAI Rank - Claude Code エージェントリーダーボード
...
>
> 本番環境の移行により、**既存のアカウントがすべてリセット**されました。以前に登録された方は、[rank.mo.ai.kr](https://rank.mo.ai.kr)で**再度会員登録**をお願いいたします。ご不便をおかけして申し訳ございません。

---

## 📚 みんなの AI エージェンティックコーディング講座

このプロジェクトは **MoAI-ADK の実際の活用を示す講座用例**として作成されました。

### 🎯 プロジェクト目的

**エージェンティックコーディングの新たな次元を体験してください！**

MoAI Rank は以下の学習目的で作られました：
...
このプロジェクトは 48 時間で MoAI-ADK を使って集中的に作られました。実戦環境での迅速な開発を示す実証例です。

**開発特徴**:

- MoAI-ADK の `/moai:alfred` `/moai:loop` コマンドで完全自律自動化コマンドで核心機能を実装
...
- エラーや改善点が見つかった場合は、[GitHub Issues](https://github.com/modu-ai/moai-rank/issues) に残してください
- コミュニティの貢献がプロジェクトをより堅牢にします

### 🔗 関連プロジェクト

- **[MoAI-ADK](https://github.com/modu-ai/moai-adk)**: AI 開発フレームワーク

---

## 🚀 始める前に: MoAI-ADK のインストール

MoAI Rank を使用するには、まず **MoAI-ADK** がインストールされている必要があります。

### MoAI-ADK とは？

MoAI-ADK は **高品質なコードを作る AI 開発フレームワーク**です。
...
MoAI-ADK がインストールされると、MoAI Rank CLI を使用できます。
...
MoAI Rank - Claude Code トークン使用量リーダーボード

Claude Code セッションを追跡し、リーダーボードで競いましょう。
ウェブダッシュボード: https://rank.mo.ai.kr

オプション:
  --help    ヘルプメッセージを表示

コマンド:
  login      GitHub OAuth で MoAI Rank にログイン (エイリアス: register)
  status     現在のランクと統計を表示
  exclude    プロジェクトをセッション追跡から除外
  include    以前除外したプロジェクトを再包含
  logout     保存された MoAI Rank 認証情報を削除
```
...
> **注記**: `moai rank register` は下位互換性のためエイリアスとして引き続きサポートされます。
...
ローカルのClaude CodeセッションデータをMoAI Rankサーバーに同期します。

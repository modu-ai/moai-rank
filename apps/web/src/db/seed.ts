...
async function seed() {
	...
	try {
		...
		for (let i = 0; i < DUMMY_USERS.length; i++) {
			...
			console.log(
				`  ✅ Created user: ${profile.username} (${profile.vibeStyle})`,
			);
		}
		...
		console.log(
			"📊 Generating session and token usage data with vibe metrics...",
		);
		const now = new Date();
		const userStats: Map<
			string,
			{ totalInput: number; totalOutput: number; sessions: number }
		> = new Map();
		...
		for (const user of createdUsers) {
			...
			for (let s = 0; s < sessionCount; s++) {
				...
				const durationSeconds = Math.floor(
					user.profile.avgSessionDuration * durationVariance,
				);
				const sessionStartDate = new Date(
					sessionEndDate.getTime() - durationSeconds * 1000,
				);

				// Turn count with variance
				const turnVariance = 0.6 + Math.random() * 0.8;
				const turnCount = Math.floor(
					user.profile.avgTurnsPerSession * turnVariance,
				);
				...
				const codeMetrics = generateCodeMetrics(
					user.profile.vibeStyle,
					turnCount,
				);
				...
				const { input, output, cacheCreation, cacheRead } =
					generateTokenUsage();
		...
		// Calculate correct periodStart for each period type (matching API logic)
